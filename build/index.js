...
const build_options = {"development":false,"dynamic_origin":true,"xff_depth":1,"assets":true};
if ("") {
...
function ssr(req) {
  ...
  if (address_header && !request.headers.has(address_header)) {
    throw new Error(
        `Address header was specified with ${"" + 'ADDRESS_HEADER'
      ...
      if (address_header) {
        ...
        if (address_header === 'x-forwarded-for') {
          ...
          if (xff_depth > addresses.length) {
            throw new Error(
                `${"" + 'XFF_DEPTH'} is ${xff_depth}, but only found ${addresses.length
                } addresses`
            );
          }
